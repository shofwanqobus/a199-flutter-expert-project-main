...
void main() {
  ...
  group('Detail TV Page', () {
    ...
    testWidgets('Should display Snackbar when state is TVAdd',
        (WidgetTester tester) async {
      when(() => mockTVDetailBloc.state)
          .thenReturn(TVDetailData(testTVDetail, testTVList));
      when(() => mockWatchlistTVStatusBloc.state)
          .thenReturn(TVStatusState(false));
      when(() => mockWatchlistTVModifyBloc.state)
          .thenReturn(TVAdd('Added to Watchlist'));
      whenListen(
          mockWatchlistTVModifyBloc,
          Stream.fromIterable([
            TVDetailLoading(),
            TVAdd('Added to Watchlist'),
          ]));
    ...
    testWidgets('Should display Snackbar when state is TVRemove',
        (WidgetTester tester) async {
      when(() => mockTVDetailBloc.state)
          .thenReturn(TVDetailData(testTVDetail, testTVList));
      when(() => mockWatchlistTVStatusBloc.state)
          .thenReturn(TVStatusState(true));
      when(() => mockWatchlistTVModifyBloc.state)
          .thenReturn(TVRemove('Removed from Watchlist'));
      whenListen(
          mockWatchlistTVModifyBloc,
          Stream.fromIterable([
            TVDetailLoading(),
            TVRemove('Removed from Watchlist'),
          ]));
      final button = find.byType(ElevatedButton);
    ...
    testWidgets(
        'Should display AlertDialog when state is WatchListTVModifyError',
        (WidgetTester tester) async {
      when(() => mockTVDetailBloc.state)
          .thenReturn(TVDetailData(testTVDetail, testTVList));
      when(() => mockWatchlistTVStatusBloc.state)
          .thenReturn(TVStatusState(false));
      when(() => mockWatchlistTVModifyBloc.state)
          .thenReturn(WatchListTVModifyError('Failed'));
      whenListen(
          mockWatchlistTVModifyBloc,
          Stream.fromIterable([
            TVDetailLoading(),
            WatchListTVModifyError('Failed'),
          ]));
      final button = find.byType(ElevatedButton);
